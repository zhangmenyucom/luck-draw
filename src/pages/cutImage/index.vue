<template>
    <div>
        ...
        <div class="wx-content-info">
            ...
            <movable-view :animation="false" :y="viewTop" :style="'width: '+width+'px;height: '+height+'px;top:0px;'" direction="all" scale scale-min="1" scale-max="2.5">
            ...
            <div class="cutTip">裁剪后图片尺寸为900*450px</div>
            <div class='cropper-config'>
                <div class="cancleButton" @tap="cancleCut"> 取消 </div>
                <div class="confirmButton" @tap="getFilePathRect"> 完成 </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    getImageInfo () {
      ...
      ctx.draw(true, () => {
        this.$canvasToTempFilePath(x, y, 375, 187.5, 900, 450, 1, 'myCanvas').then(res => {
          return this.$uploadFile(res.tempFilePath)
        }).then((res) => {
          console.log('res', res)
          if (res.code === 0) {
            this.$setStorageSync('cutImageSrc', res.data.url)
            this.$setStorageSync('cutPicIndex', this.picIndex)
            this.$navigateBack(1)
          }
        })
      })
  ...
  onLoad (options) {
    this.imageSrc = options.imgSrc
    this.picIndex = options.picIndex
